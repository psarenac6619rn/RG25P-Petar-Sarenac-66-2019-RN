...
// Mreza sazvezdja: svaka proceduralna zvezda se povezuje sa susedima.
// Linije su namerno samo u DMT modu.
float constellationNetwork(vec2 uv){
    ...
    for(int y=-1;y<=1;y++){
        for(int x=-1;x<=1;x++){
            ...
            // Ne povezujemo bas svaki cvor - tako mreza izgleda organski.
...
// Simetricna "sacred geometry" mandala iznad glave.
// Polarno preslikavanje stvara mnogo ponovljenih fraktalnih krakova.
float zenithMandala(vec2 uv){
    ...
    // Rekurzivni/kaleidoskopski unutrasnji detalj.
    vec2 q = p;
    float fractal = 0.0;
    for(int i=0;i<4;i++){
...
vec3 proceduralSky(vec2 uv){
    ...
    // Gusta zvezdana pozadina: sitne + retke svetlije zvezde.
    ...
    if(dmtMode==1){
        ...
        // Veliki fraktalni "portal" direktno na zenitu.
        ...
        // Vertikalna nit energije od centra ekrana ka mandali.
        ...
        // Blagi halo oko portala.
        vec2 zp=(uv-vec2(.5,.86))*vec2(1.78,1.0);
        sky += vec3(.22,.08,.32)*exp(-dot(zp,zp)*9.0)*0.30;

        // During F2 launch the zenith source becomes increasingly dominant.
...
void main(){
    ...
    if(dmtMode==1){
        ...
        // Svetle linije po zelenim/braon "zivim" materijalima drveta.
        ...
        // Procedural flowers use magenta/purple albedo; when one is being observed
        // their emissive fractal response increases together with camera focus.
